...
**_Note:_**
_The expression must be a simple command.
...
When used, the assertion fails if the _extended regular expression_ does not match `$output`.

_Note: The anchors `^` and `$` bind to the beginning and the end of the entire output (not individual lines), respectively._
...
**_Warning:_**
_Due to a [bug in Bats][bats-93], empty lines are discarded from `${lines[@]}`, causing line indices to change and preventing testing for empty lines._
...
**_Warning:_**
_The output displayed does not contain empty lines.
...
_Note: As expected, the anchors `^` and `$` bind to the beginning and the end of the matched line, respectively._
